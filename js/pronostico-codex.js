...
function codexPsychologyProfile(team) {
  const fallback = {
    motivazione: 80,
    pressione: 65,
    momentoEmotivo: 0,
    notaPsicologica: "Profilo mentale equilibrato, senza segnali estremi di pressione o slancio emotivo.",
  };
  const profile = typeof psychologicalFactors !== "undefined"
    ? psychologicalFactors[team] || fallback
    : fallback;
  return { team, ...profile };
}
...
function codexScoreMatch(teamA, teamB, knockout = false, fixture = null, matchNumber = 0) {
  ...
  const psychologyA = codexPsychologyModifier(teamA, matchNumber);
  const psychologyB = codexPsychologyModifier(teamB, matchNumber);
  expectedA = codexClamp(expectedA * (1 + psychologyA), 0.03, 3.3);
  expectedB = codexClamp(expectedB * (1 + psychologyB), 0.03, 3.3);
  if (mismatch > 300) {
...
function codexSimulateGroups() {
  ...
  fixtures.forEach((fixture, index) => {
    ...
    const forecast = codexScoreMatch(teams[0], teams[1], false, fixture, index + 1);
...
function codexSignedPercent(value) {
  const percent = Number(value || 0) * 100;
  return `${percent >= 0 ? "+" : ""}${percent.toFixed(1)}%`;
}

function codexEmotionLabel(value) {
...
function codexMotivationBadge(team) {
  ...
  const tone = motivation.modifier > 0.025 ? "is-positive" : motivation.modifier < 0 ? "is-negative" : "is-neutral";
  return `<span class="codex-motivation-mini ${tone}" title="${codexEscape(motivation.notaPsicologica)}">&#129504; ${codexSignedPercent(motivation.modifier)}</span>`;
}

function codexRenderMotivationRanking() {
  const root = document.getElementById("codex-motivation-ranking");
  if (!root) return;
  const rows = codexMotivationRanking();
...
function codexPsychologyMatchPanel(result, matchNumber, compact = false) {
  const source = result?.isReal && result.forecast ? result.forecast : result;
  if (!source?.teamA || !source?.teamB) return "";
  const teams = [source.teamA, source.teamB];
  const rows = teams.map((team) => {
    const profile = codexMotivationIndex(team, matchNumber);
    const modifier = team === source.teamA
      ? source.psychologyA ?? profile.modifier
      : source.psychologyB ?? profile.modifier;
    return { ...profile, modifier };
  });
  if (compact) {
    return `<div class="codex-world-psychology">${rows.map((row) => `
  ...
  return `
    <div class="codex-psychology-match">
      <strong>Fattore mentale${matchNumber >= 89 ? " · impatto knockout 1,3x" : ""}</strong>
      <div>${rows.map((row) => `
        <article title="${codexEscape(row.notaPsicologica)}">
          <span>${codexFlag(row.team)}${codexEscape(row.team)}</span>
          <small>M ${row.motivazione} · P ${row.pressione} · Umore ${codexEmotionLabel(row.momentoEmotivo)}</small>
          <b class="${row.modifier >= 0 ? "is-positive" : "is-negative"}">${codexSignedPercent(row.modifier)}</b>
        </article>`).join("")}</div>
    </div>`;
}
...
function codexWorldMatchCard(matchNumber, compact = false) {
  ...
  return `
    <article class="codex-world-match ${compact ? "is-compact" : ""} ${result.isReal ? "is-real-result" : ""}">
      ...
      ${codexPsychologyMatchPanel(result, matchNumber, true)}
    ...
    <div class="codex-world-scroll">
      <div class="codex-world-board">
        <div class="codex-world-title">
          ...
          <span>Pronostico Codex+ 2026 · Codex 70% · Opta 12% · Klement 7% · Goldman 6% · IA 5% · fattore mentale applicato per partita</span>
        </div>
