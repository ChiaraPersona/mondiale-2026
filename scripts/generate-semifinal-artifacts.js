...
const data = [
  {
    ...
    round: "Semifinale",
    lineupsStatus: "ufficiali",
    home: { name: "Francia", flag: "francia", formation: "4-2-3-1", line: "Maignan; Digne, Saliba, Upamecano, Kounde; Tchouameni, Rabiot; Barcola, Olise, Dembele; Mbappe.", pitch: [["Mbappe"], ["Barcola", "Olise", "Dembele"], ["Tchouameni", "Rabiot"], ["Digne", "Saliba", "Upamecano", "Kounde"], ["Maignan"]] },
    away: { name: "Spagna", flag: "spagna", formation: "4-2-3-1", line: "Simon; Porro, Cubarsi, Laporte, Cucurella; Rodri, Fabian Ruiz; Lamine Yamal, Olmo, Baena; Oyarzabal.", pitch: [["Oyarzabal"], ["Baena", "Olmo", "Lamine Yamal"], ["Fabian Ruiz", "Rodri"], ["Cucurella", "Laporte", "Cubarsi", "Porro"], ["Simon"]] },
    headline: "Ufficiali piu equilibrate: la Francia alza Barcola, la Spagna sceglie controllo e mezzo spazio con Baena.",
    ...
    scenario: "La Spagna conserva piu controllo centrale, ma senza Nico perde profondita pura a sinistra; la Francia con Barcola e Dembele puo attaccare meglio entrambi i lati.",
    ...
    refereeIntelligence: {
      ...
      modelImpact: "Aumenta leggermente cartellini nel secondo tempo, falli tattici, proteste e profili di centrocampo/difesa. Non aumenta artificialmente il rigore: Mbappe, Dembele, Barcola, Olmo e Lamine Yamal restano il fattore primario."
    },
    lead: "Questa e una semifinale da leggere piu con la posizione che con il puro volume. Le ufficiali riducono lo scarto: la Spagna resta piu ordinata con Rodri-Fabian Ruiz e Olmo tra le linee, ma l'assenza di Nico Williams toglie campo aperto sul lato debole. La Francia guadagna ampiezza reale con Barcola e Dembele attorno a Mbappe. Il risultato centrale resta 1-1, con qualificazione quasi al 50/50.",
    paragraphs: [
      "La Francia deve accettare fasi lunghe senza palla senza perdere aggressivita quando recupera. Con Digne al posto di Theo Hernandez c'e meno strappo lungo da terzino, quindi diventano piu importanti le ricezioni alte di Barcola e Dembele. Se Mbappe riceve fronte alla porta o puo attaccare Cubarsi/Laporte in campo aperto, ogni transizione diventa una giocata da semifinale.",
      "La Spagna deve evitare di trasformare il possesso in sterile sicurezza. Rodri e Fabian Ruiz sono il termometro, mentre Baena aggiunge pulizia e tiro da seconda linea ma meno minaccia in campo aperto rispetto a Nico Williams. Per questo la MyCombo resta su Goal controllato e mercati volume, senza forzare un segno secco nei novanta minuti."
    ],
    ...
    tacticalKey: "La chiave e il primo vantaggio territoriale: Spagna per possesso e rifinitura centrale, Francia per strappi dei tre alle spalle di Mbappe. Con ambiente climatizzato non c'e premio fisico al risparmio: chi tiene meglio le distanze puo alzare ritmo anche nella ripresa.",
    teamASection: "La Francia deve difendere in avanti quando puo e non lasciare a Rodri il tempo di girarsi. Rabiot e Tchouameni hanno una doppia funzione: proteggere il centro e liberare la prima uscita verso Barcola, Olise e Dembele. Digne rende la catena sinistra piu prudente, quindi il cambio ritmo deve arrivare prima dagli esterni alti.",
    teamBSection: "La Spagna deve manipolare il lato forte e poi cambiare gioco con pulizia. Lamine Yamal resta il punto di superiorita piu naturale, Olmo attacca tra le linee e Baena puo dare un tiro in piu da zona sinistra; senza Nico Williams, pero, cala la profondita immediata alle spalle di Kounde.",
    shots: "Francia: 12-15 tiri, 4-6 in porta, 4-6 corner. Spagna: 12-15 tiri, 4-6 in porta, 4-6 corner. Proiezioni individuali: Mbappe 4-6 tiri e primo riferimento per tiri in porta; Lamine Yamal 2-4 tiri; Barcola/Dembele 1-3; Olmo e Baena 1-3 complessivi.",
    volumeSummary: { home: "12-15 tiri", away: "12-15 tiri", total: "24-30 tiri" },
    cards: [
      ...
      ["Digne", "Medio", "Duello con Lamine Yamal: rischio intervento in ritardo se la Spagna isola il lato destro."],
    ...
    valueErrors: [
      ...
      ["Pareggio + Over 1,5 @ 3.75", "COMBO: 1X2 + U/O", "Le ufficiali avvicinano ulteriormente il 90': Francia piu ampia davanti, Spagna piu controllata ma meno profonda a sinistra."],
...
function article(config, portfolios) {
  const lineupsStatus = config.lineupsStatus || "probabili";
  const lineupsTitle = lineupsStatus === "ufficiali" ? "Formazioni ufficiali" : "Formazioni probabili";
  const lineupNote = lineupsStatus === "ufficiali" ? "Ufficiale: integrata nel ricalcolo lettura/MyCombo." : "Probabile, da aggiornare alle ufficiali.";
  ...
  const formation = (team) => `<article class="match-formation-card"><header><img src="flags/${team.flag}.svg" alt=""><div><strong>${esc(team.name)}</strong><span>Modulo ${esc(team.formation)} &middot; ${esc(lineupsStatus)}</span></div></header><div class="football-pitch" aria-label="${esc(lineupsTitle)} ${esc(team.name)}">${team.pitch.map((row) => `<div class="pitch-row">${row.map((name) => `<span>${esc(name)}</span>`).join("")}</div>`).join("")}</div><div class="formation-bench"><b>Nota</b><span>${esc(lineupNote)}</span></div></article>`;
...
function readingJson(config, portfolios) {
  return {
    ...
    formations: {
      status: config.lineupsStatus || "probabili",
...
function myComboJson(config, quote, portfolios) {
  const payload = {
    ...
    qualityNotes: [
      ...
      config.lineupsStatus === "ufficiali" ? "Formazioni ufficiali integrate nel ricalcolo." : "Formazioni probabili da aggiornare alle ufficiali."
    ]
  };
  return payload;
}
